...
namespace Yodo1.MAS
{
    ...
    /// </summary>
    public abstract class Yodo1ProcessGradleBuildFile
    {
        ...
        protected bool AddPluginToRootGradleBuildFile(string rootGradleBuildFile)
        {
            ...
            {
                if (line.Contains("com.applovin.quality"))
                {
                    return true;
                }
            }
            ...
            if (!pluginAdded)
            {
                Debug.LogError(Yodo1U3dMas.TAG + "Failed to add AppLovin Quality Service plugin to root gradle file.");
                return false;
            }
        ...
        protected bool AddAppLovinRepository(string settingsGradleFile)
        {
            ...
            foreach (var line in lines)
            {
                if (line.Contains("https://artifacts.applovin.com/android"))
                {
                    return true;
                }
            }
            ...
            if (!mavenRepoAdded)
            {
                Debug.LogError(Yodo1U3dMas.TAG + "Failed to add AppLovin Quality Service plugin maven repo to settings gradle file.");
                return false;
            }
            ...
            {
                Debug.LogError(Yodo1U3dMas.TAG + "Failed to install AppLovin Quality Service plugin. Setting Gradle file write failed.");
                Console.WriteLine(exception);
                return false;
            }
        ...
        protected bool AddMasRepository(string settingsGradleFile)
        {
            var lines = File.ReadLines(settingsGradleFile).ToList();
            var outputLines = new List<string>();
            var mavenRepoAdded = false;
            var dependencyResolutionManagementClosureDepth = 0;
            var insideDependencyResolutionManagementClosure = false;
            var dependencyResolutionManagementMatched = false;

            foreach (var line in lines)
            {
                outputLines.Add(line);

                if (!dependencyResolutionManagementMatched && line.Contains(DependencyResolutionManagementMatcher))
                {
                    dependencyResolutionManagementMatched = true;
                    insideDependencyResolutionManagementClosure = true;
                }

                if (insideDependencyResolutionManagementClosure)
                {
                    if (line.Contains("{"))
                    {
                        dependencyResolutionManagementClosureDepth++;
                    }
                    ...
                    if (dependencyResolutionManagementClosureDepth == 0)
                    {
                        insideDependencyResolutionManagementClosure = false;
                    }
                }

                if (insideDependencyResolutionManagementClosure)
                {
                    Regex TokenBuildScriptRepositories = new Regex(".*repositories {.*");

                    if (!mavenRepoAdded && TokenBuildScriptRepositories.IsMatch(line))
                    {
                        List<string> mavenUrls = GetAllMasMavenUrls();
                        foreach (string mavenUrl in mavenUrls)
                        {
                            outputLines.Add(GetFormattedBuildScriptLine(mavenUrl));
                        }
            ...
            {
                Debug.LogError(Yodo1U3dMas.TAG + "Failed to install MAS plugin. Setting Gradle file write failed.");
                Console.WriteLine(exception);
                return false;
            }
        ...
        protected List<string> GetAllMasMavenUrls()
        {
            List<string> mavenUrlList = new List<string>();
            ...
            XmlNode repositoriesRead = androidPackagesRead.SelectSingleNode("repositories");
            XmlNodeList repositoryList = repositoriesRead.SelectNodes("repository");
            if (repositoryList == null)
            {
                return mavenUrlList;
            }

            foreach (XmlNode repository in repositoryList)
            {
                string url = "maven { url '" + repository.InnerText + "' }";
                mavenUrlList.Add(url);
            }
        ...
        protected static void RemoveAppLovinQualityServiceOrSafeDkPlugin(string gradleTemplateFile)
        {
            ...
            {
                Debug.LogError(Yodo1U3dMas.TAG + "Failed to remove AppLovin Quality Service Plugin from mainTemplate.gradle. Please remove the Quality Service plugin from the mainTemplate.gradle manually.");
                Console.WriteLine(exception);
            }
        }
#endif
