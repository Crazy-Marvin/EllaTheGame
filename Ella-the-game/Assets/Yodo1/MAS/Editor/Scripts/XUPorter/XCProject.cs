...
namespace UnityEditor.XCodeEditor
{
	public partial class XCProject : System.IDisposable
	{
		...
		public void SetNativeTarget( string name, PBXNativeTarget target)
		{
			foreach (string guid in nativeTargets.Keys)
            {
				PBXNativeTarget currentObject = nativeTargets[guid];
				if ((string)currentObject.data["name"] == name)
                {
					nativeTargets[guid] = target;
					break;
                }
			}
		}
		...
		public void Save()
		{
			PBXDictionary result = GetResult();
